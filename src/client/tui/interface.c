		...
		if (ch == 'q' || ch == 27) {
			...
			if (getmouse(&local_event) == OK) {
				if (local_event.y == btn_y && 
				    ...
				    (local_event.bstate & (BUTTON1_CLICKED | BUTTON1_PRESSED))) {
					running = false;
				}
				
#ifdef BUTTON4_PRESSED
				if (local_event.bstate & BUTTON4_PRESSED) scroll--;
#endif
#ifdef BUTTON5_PRESSED
				if (local_event.bstate & BUTTON5_PRESSED) scroll++;
#endif
			}
		}
	}
...
void popup_execute_cmd(void)
{
	...
	if (strlen(buf) > 0) {
		...
		if (output) {
			if (core_execute_command(current_server.ip, current_server.port, buf, output, OUTPUT_BUFFER_SIZE) == 0) {
				popup_show_output("STDOUT/STDERR", output);
			} else {
				popup_show_output("ERROR", "Failed to execute command or receive response.");
			}
			free(output);
		}
	}
